...
export function useUserBrands(
  userId: string,
  options?: Omit<UseQueryOptions<api.Brand[], Error>, "queryKey" | "queryFn">,
) {
  return useQuery({
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: ["brands", "user", data.user_id],
      });
    },
  });
}
...
export function useQueuePosts() {
  return useMutation({
    mutationFn: (input: {
      userId: string;
      brandId: string;
      ideas: { message: string; idea_type: string }[];
    }) => api.queuePosts(input.userId, input.brandId, input.ideas as any),
  });
}
...
export function useDeleteCategory(brandId: string) {
  ...
  return useMutation({
    mutationFn: (categoryId: string) => api.deleteCategory(brandId, categoryId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["categories", brandId] });
    },
  });
